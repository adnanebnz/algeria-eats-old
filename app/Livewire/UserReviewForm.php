...
class UserReviewForm extends Component
{
    ...
    public function store()
    {
        ...
        $dealedWithThisDeliveryMan = Delivery::whereHas('order', function ($query) {
            $query->where('buyer_id', auth()->user()->id)->orWhere('artisan_id', auth()->user()->id);
        })->whereHas('deliveryMan', function ($query) {
            $query->where('user_id', $this->user->id);
        })->first();


        if ($purchasedFromThisUser === null && $dealedWithThisDeliveryMan === null) {
            Alert::error('Erreur', "Vous n'avez rien acheté ou n'avez rien eu avec cet utilisateur");
