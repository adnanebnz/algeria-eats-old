                                ...
                                @forelse ($deliveries as $delivery)
                                    <tr>
                                            ...
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 w-10 h-10">
                                                    <img class="w-full h-full rounded-full border"
                                                        src="{{ $delivery->deliveryMan->image ? (str_starts_with($delivery->deliveryMan->image, 'http') ? $delivery->deliveryMan->image : asset('storage/' . $delivery->deliveryMan->image)) : asset('assets/user.png') }}" />
                                                </div>
                                                <div class="ml-3">
                                                    <p class="text-gray-900 whitespace-no-wrap">
                                                        {{ $delivery->deliveryMan->getFullName() }}
                                                    </p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-5 py-5 border-gray-200 bg-white text-sm">
                                            @foreach ($delivery->orderItems as $item)
                                                <p class="text-gray-900 whitespace-no-wrap">
                                                    {{ $item->product->nom }} (x{{ $item->quantity }})
                                                </p>
                                                @if ($delivery->orderItems->count() > 1 && !$loop->last)
                                                    <hr class="my-2">
                                                @endif
                                            @endforeach
                                        </td>
                                        ...
                                        <td
                                            ...
                                            <a href="{{ route('artisan.deliveries.show', ['delivery' => $delivery]) }}"
                                            ...
                                            <div x-data="{ tooltipVisible: false }">
                                                <form
                                                    action="{{ route('artisan.deliveries.invoice', ['delivery' => $delivery]) }}"
                                                    ...
                                                    <div x-show="tooltipVisible"
                                                        class="absolute right-0 hidden md:left-2/3 md:top-72 z-10 md:inline-block w-64 text-sm text-gray-500 transition-opacity duration-300 bg-white border border-gray-200 rounded-lg shadow-sm opacity-100">
                                                        <div
                                            ...
                                            <form method="POST"
                                                action="{{ route('artisan.deliveries.destroy', ['delivery' => $delivery]) }}"
