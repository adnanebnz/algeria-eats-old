<div>
    <section class="bg-white"
             x-data="{ artisanRating: @entangle('artisanRating'), productRating: @entangle('productRating') }">
                ...
                <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-md">
                    <form wire:submit.prevent="applyFilters">
                        ...
                        <div
                            ...
                            <x-select name="artisanWilaya" label="Wilaya d'artisan" :list="$wilayas"
                                      :optionsValues="'wilaya_name_ascii'"
                            ...
                            <div class="flex flex-col">
                                ...
                                <select id="manufacturer" name="typeService" wire:model="typeService"
                                    ...
                                    <option value="sucree_salee"
                                            @if (request()->typeService == 'sucree_salee') selected @endif>Sucrée et
                                        Salée
                                    </option>
                                </select>
                            </div>
                        ...
                        <div class="mt-6 grid w-full grid-cols-2 justify-end space-x-4 md:flex">
                            ...
                            <button
                                class="rounded-lg bg-blue-600 md:px-8 px-2 py-2 font-medium text-white outline-none hover:opacity-80 focus:ring">
                                Rechercher
                            </button>
                        </div>
                    </form>
                            ...
                            <a href="{{ route('profile', $artisan->user->id) }}"
                               class="relative block overflow-hidden shadow-lg border p-3 hover:opacity-80">
                                <div class="flex flex-row items-center gap-2">
                                    <img
                                        src="{{ $artisan->user->image ? (str_starts_with($artisan->user->image, 'http') ? $artisan->user->image : asset('storage/' . $artisan->user->image)) : asset('assets/user.png') }}"
                                        class="object-cover w-12 h-12 rounded-full border"
                                        alt="artisan profile picture"/>
                                    <div class="flex flex-col justify-center">
                                        <p class="ml-2 text-sm font-medium text-gray-700">{{ $artisan->user->getFullName() }}</p>
