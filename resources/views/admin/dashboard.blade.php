   ...
   <div class="flex gap-2 my-2">
     ...
     @livewire('user-statistic')
      <div class="w-1/2 h-max bg-slate-200 hover:bg-slate-300 rounded-lg p-2 text-3xl">
         ...
         <h1 class="my-2">{{$artisans->count()}} Artisan : {{(($artisans->count())/($users->count()))*100}} %</h1>
         <h1 class="my-2">{{$deliveryMans->count()}} Livreur : {{(($deliveryMans->count())/($users->count()))*100}} %</h1>
         <h1 class="my-2">{{$consumer->count()}} Consumer : {{(($consumer->count())/($users->count()))*100}} % </h1>
     </div>
